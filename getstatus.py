...
class Growatt:
    ...
    def getsigned(self, regbyte, intlen = 16):
    ...
    def read(self):
        ...
        intlen = 16
        #totbattwatts = row.registers[78]
        #unsigned = totbattwatts % 2**intlen
        #signedbattwatt = unsigned - 2**intlen if unsigned >= 2**(intlen-1) else unsigned
        ...
        info = {                                    # ==================================================================
            ...
            "Bat_wattH": float( self.getsigned(row.registers[77]) ) / 10,    # W      Signed int positive discharge, negative battery charge power
            "Bat_wattL": float( self.getsigned(row.registers[78]) ) / 10,   # W      Signed int positive discharge, negative battery charge power
